...
// Show/hide "Other" device type text field
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('deviceType').addEventListener('change', function() {
        const otherGroup = document.getElementById('otherDeviceGroup');
...
function submitDevice(event) {
    ...
    if (type === 'other') {
    type = document.getElementById('otherDevice').value.toLowerCase().trim();
    }
    ...
    deviceList.innerHTML = userData.devices.map((device, index) => `
        <div class="device-item" style="animation-delay: ${index * 0.1}s">
            <div class="device-header">
                <div>
                   <strong class="device-name">${device.type}</strong> -<span class="device-brand"> ${device.brand}</span>
                    <br>
                    <span class="device-id">${device.id}</span>
                </div>
